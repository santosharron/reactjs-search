...
class Graph
{
    public function allPosts($post_types, $wpml_lang)
    {
        ...
        return apply_filters('reactivepro_allPosts', $results);
    }
    ...
    public function get_all_terms($taxonomies, $post_types, $wpml_lang, $restricted_terms)
    {
        ...
        if (empty($restricted_terms)) {
            $restricted_terms = array('dksjdksdh');
        }
      ...
      GROUP BY {$wpdb->term_taxonomy}.term_id", $taxonomies);
        ...
        return apply_filters('reactivepro_get_all_terms', $results);
    }

    public function get_rnb_terms($taxonomies, $post_types, $wpml_lang, $restricted_terms)
    {
        global $wpdb;
        $rnb_pivot_table = $wpdb->prefix . 'rnb_inventory_product';
        $wpdb->get_results("SET SESSION group_concat_max_len = 999999999999999", 'ARRAY_A');
        if (empty($taxonomies)) {
            $taxonomies = array('category');
        }
        ...
        $taxonomies[]      = 'inherit';
        if ($wpml_lang === '') {
            $query = $wpdb->prepare("SELECT CASE WHEN {$wpdb->posts}.post_type = 'inventory' THEN GROUP_CONCAT(DISTINCT product) ELSE GROUP_CONCAT(DISTINCT object_id) END  as allPosts, {$wpdb->terms}.term_id, slug, taxonomy
        FROM {$wpdb->term_relationships}
        LEFT JOIN {$wpdb->term_taxonomy}
        ON {$wpdb->term_relationships}.term_taxonomy_id = {$wpdb->term_taxonomy}.term_taxonomy_id
        LEFT JOIN {$wpdb->posts}
        ON {$wpdb->term_relationships}.object_id = {$wpdb->posts}.ID
        LEFT JOIN {$rnb_pivot_table}
        ON {$wpdb->term_relationships}.object_id = {$rnb_pivot_table}.inventory
        LEFT JOIN {$wpdb->terms}
        ON {$wpdb->terms}.term_id = {$wpdb->term_taxonomy}.term_id
        WHERE  {$wpdb->term_taxonomy}.taxonomy IN ($taxonomy_placeholder)
        AND {$wpdb->posts}.post_type IN ($post_type_placeholder)
      ...
      GROUP BY {$wpdb->term_taxonomy}.term_id", $taxonomies);
        ...
        return apply_filters('reactivepro_get_rnb_terms', $results);
    }
    ...
    public function get_term_to_term($taxonomies, $post_types, $wpml_lang = '')
    {
      ...
      GROUP BY firstTermTax.parent", $taxonomies);
        ...
        return apply_filters('reactivepro_get_term_to_term', $results);
    }
    ...
    public function get_term_to_post($taxonomies, $post_types, $wpml_lang)
    {
      ...
      GROUP BY {$wpdb->term_taxonomy}.term_id", $taxonomies);
        ...
        return apply_filters('reactivepro_get_term_to_post', $results);
    }
    ...
    public function get_meta_to_post($metakeys, $wpml_lang, $post_types, $metakeys_type_data)
    {
      ...
      GROUP BY {$wpdb->postmeta}.meta_value, {$wpdb->postmeta}.meta_key", $metakeys);
        ...
        return apply_filters('reactivepro_get_meta_to_post', $results);
    }

    public function rnb_meta_to_post($metakeys, $wpml_lang, $post_types, $metakeys_type_data)
    {
        global $wpdb;
        $rnb_pivot_table = $wpdb->prefix . 'rnb_inventory_product';
        $wpdb->get_results("SET SESSION group_concat_max_len = 999999999999999", 'ARRAY_A');
        ...
        $metakeys[]             = 'publish';
        $metakeys[]             = 'inherit';
        $query                  = $wpdb->prepare("SELECT CASE WHEN {$wpdb->posts}.post_type = 'inventory' THEN GROUP_CONCAT(DISTINCT product) ELSE GROUP_CONCAT(DISTINCT post_id) END as allPosts, meta_value, meta_key
      FROM {$wpdb->postmeta}
      LEFT JOIN {$wpdb->posts}
      ON {$wpdb->postmeta}.post_id = {$wpdb->posts}.ID
      LEFT JOIN {$rnb_pivot_table}
      ON {$wpdb->postmeta}.post_id = {$rnb_pivot_table}.inventory
      WHERE {$wpdb->postmeta}.meta_key IN ($metakey_placeholder)
      AND {$wpdb->posts}.post_type IN ($post_types_placeholder)
      AND {$wpdb->postmeta}.meta_value <> ''
      AND ({$wpdb->posts}.post_status = %s OR {$wpdb->posts}.post_status = %s)
      GROUP BY {$wpdb->postmeta}.meta_value, {$wpdb->postmeta}.meta_key", $metakeys);
        ...
        return apply_filters('reactivepro_rnb_meta_to_post', $results);
    }
    ...
    public function get_taxonomy_to_term($taxonomies, $post_types, $wpml_lang)
    {
      ...
      GROUP BY taxonomy", $taxonomies);
        ...
        return apply_filters('reactivepro_get_taxonomy_to_term', $results);
    }
    ...
    public function get_term_names($wpml_lang, $restricted_terms, $taxonomies)
    {
        ...
        $wpdb->get_results("SET SESSION group_concat_max_len = 999999999999999", 'ARRAY_A');
        if (empty($restricted_terms)) {
            $restricted_terms = array('category');
        }
        $terms_placeholder = implode(', ', array_fill(0, count($restricted_terms), '%s'));
        if (empty($taxonomies)) {
      ...
      AND t_translation.language_code = %s GROUP BY t_term.term_id", $restricted_terms);
        ...
        return apply_filters('reactivepro_get_term_names', $results);
    }
    ...
    public function get_all_terms_meta_key()
    {
        ...
        return apply_filters('reactivepro_get_all_terms_meta_key', $all_term_meta_key);
    }
    ...
    GROUP BY {$wpdb->termmeta}.meta_value, {$wpdb->termmeta}.meta_key", $term_meta_key);
        ...
        return apply_filters('reactivepro_get_term_meta_to_term_data', $results);
    }
    ...
    public function text_search_result($text_search_string)
    {
        ...
        $post_query              = $wpdb->prepare(
            "SELECT GROUP_CONCAT(DISTINCT ID) as searchResults
     FROM {$wpdb->posts}
     ...
     OR post_excerpt LIKE %s",
        ...
        $meta_query              = $wpdb->prepare(
            "SELECT GROUP_CONCAT(DISTINCT post_id) as searchResults
      FROM {$wpdb->postmeta}
      WHERE meta_value LIKE %s",
        ...
        $term_query              = $wpdb->prepare(
            "SELECT GROUP_CONCAT(object_id) as searchResults
      FROM {$wpdb->term_relationships}
      LEFT JOIN {$wpdb->terms}
      ...
      WHERE {$wpdb->terms}.name LIKE %s",
        ...
        return apply_filters('reactivepro_text_search_result', $allSearchResult);
    }
    ...
    public function sort_by_bp_activity_meta_key_value($sort_key)
    {
      ...
      ORDER BY CASE WHEN concat('',metaTbale.meta_value * 1) = metaTbale.meta_value THEN ABS(metaTbale.meta_value) ELSE LENGTH(metaTbale.meta_value) END ASC", $sort_key);
        ...
        return apply_filters('reactivepro_sort_by_bp_activity_meta_key_value', $results);
    }
    ...
    public function sorting_by_bp_activity_key($sort_key)
    {
        ...
        $results = $wpdb->get_results($query, 'ARRAY_A');
        return apply_filters('reactivepro_sorting_by_bp_activity_key', $results);
    }

    public function get_geo_search_result($address, $radius)
    {
        $locationData = $this->get_latlang($address);
        extract($locationData);
        if ($location) {
            $this->location  = $location;
            $map_search_data = location_posts_where($geoData, $location, $radius);
            return apply_filters('reactivepro_get_geo_search_result', $map_search_data);
        }
    }
        ...
        if (isset($map_api_key) && !empty($map_api_key)) {
            ...
            if ($request && !isset($request->errors)) {
                ...
                if (isset($geo) && !empty($geo)) {
                    $geoData = $geo->results[0];
                    $location      = $geo->results[0]->geometry->location;

                    return apply_filters('reactivepro_get_latlang', array('location' => $location, 'geoData' => $geoData));
                }
            }
        } else {
            return apply_filters('reactivepro_get_latlang', array());
        }
    }
    ...
    public function get_all_restricted_metas()
    {
        ...
        return apply_filters('reactivepro_get_all_restricted_metas', $restricted_metas_array);
    }
    ...
    public function get_rental_product_data()
    {
      ...
      WHERE meta_key = %s", 'redq_block_dates_and_times');
        ...
        return apply_filters('reactivepro_get_rental_product_data', $rental_boocked_date);
    }
    ...
    public function auto_complete_search($post_types, $text_to_search)
    {
      ...
      OR post_content LIKE %s) LIMIT 10", $post_types);
        ...
        return apply_filters('reactivepro_auto_complete_search', $result);
    }
    ...
    public function get_post_to_terms($post_types, $taxonomies, $wpml_lang = '', $restricted_terms)
    {
        ...
        return apply_filters('reactivepro_get_post_to_terms', $results);
    }
    ...
    public function process_sorting_data($post_types, $sorting_attributes)
    {
        ...
        foreach ($sorting_attributes as $single_attribute_key => $single_attribute) {
            switch ($single_attribute) {
                case 'post_title':
                    $sorting_result[$single_attribute] = sorting_by_post_key($post_types, $single_attribute);
                    break;
                case 'menu_order':
                    $sorting_result[$single_attribute] = sorting_by_post_key($post_types, $single_attribute);
                    break;
                ...
                case 'comment_count':
                    ...
                    break;
                default:
                    $sorting_result[$single_attribute] = sort_by_meta_key_value($post_types, $single_attribute);
                    break;
            }
        }

        return apply_filters('reactivepro_process_sorting_data', $sorting_result);
    }
        ...
        foreach ($sorting_attributes as $single_attribute_key => $single_attribute) {
            switch ($single_attribute) {
                case 'comment_content':
                    $sorting_result[$single_attribute] = sorting_by_review_key($single_attribute);
                    break;
                case 'comment_date':
                    $sorting_result[$single_attribute] = sorting_by_review_key($single_attribute);
                    break;
                case 'comment_author':
                    ...
                    break;
                default:
                    $sorting_result[$single_attribute] = sort_by_review_meta_key_value($single_attribute);
                    break;
            }
        }

        return apply_filters('reactivepro_process_review_sorting_data', $sorting_result);
    }
    ...
    public function process_bp_group_sorting_data($sorting_attributes)
    {
        ...
        foreach ($sorting_attributes as $single_attribute_key => $single_attribute) {
            if (in_array($single_attribute, $sorting_columns)) {
                $sorting_result[$single_attribute] = sorting_by_bp_group_key($single_attribute);
            } else {
                $sorting_result[$single_attribute] = sort_by_bp_group_meta_key_value($single_attribute);
            }
        }

        return apply_filters('reactivepro_process_bp_group_sorting_data', $sorting_result);
    }
    ...
    public function process_bp_activity_sorting_data($sorting_attributes)
    {
        ...
        return apply_filters('reactivepro_process_bp_activity_sorting_data', $sorting_result);
    }
    ...
    public function process_user_sorting_data($sorting_attributes)
    {
        ...
        foreach ($sorting_attributes as $single_attribute_key => $single_attribute) {
            switch ($single_attribute) {
                case 'user_login':
                    $sorting_result[$single_attribute] = sorting_by_user_key($single_attribute);
                    break;
                case 'user_nicename':
                    $sorting_result[$single_attribute] = sorting_by_user_key($single_attribute);
                    break;
                ...
                case 'display_name':
                    ...
                    break;
                default:
                    $sorting_result[$single_attribute] = sort_by_user_meta_key_value($single_attribute);
                    break;
            }
        }

        return apply_filters('reactivepro_process_user_sorting_data', $sorting_result);
    }
    ...
    public function get_term_meta_to_term($term_meta_keys)
    {
        ...
        return apply_filters('reactivepro_get_term_meta_to_term', $term_meta_to_terms);
    }
    ...
    public function get_attachment_dir_url()
    {
        ...
        if (false === $upload_dir_url_array['error']) {
            return apply_filters('reactivepro_get_attachment_dir_url', $upload_dir_url_array['baseurl']);
        } else {
            return apply_filters('reactivepro_get_attachment_dir_url', site_url() . '/wp-content/uploads');
        }
    }
    ...
    public function get_permalink_structure($post_types)
    {
        ...
        return apply_filters('reactivepro_get_permalink_structure', $rewrite_rule);
    }

    public function get_grid_all_data($post_types, $wpml_lang, $skip_post_content, $fetch_thumb_image)
    {
        global $wpdb;
        $wpdb->get_results("SET SESSION group_concat_max_len = 999999999999999", 'ARRAY_A');
        if (empty($post_types)) {
      ...
      WHERE ID IN ($attachment_placeholder) AND post_type = %s ", $attachment_ids);
        ...
        return apply_filters('reactivepro_get_grid_all_data', [
            'allPosts'          => $allPosts,
    ...
    public function get_all_users()
    {
     ...
     WHERE ID <> %s", null);
        ...
        return apply_filters('reactivepro_get_all_users', $results);
    }
    ...
    public function get_user_grid_data($wpml_lang)
    {
        ...
        return apply_filters('reactivepro_get_user_grid_data', array(
            'allPosts'          => $results,
    ...
    public function get_review_grid_data($wpml_lang, $post_types)
    {
      ...
      WHERE postTable.post_type IN ($post_types_placeholder)", $post_types);
        ...
        return apply_filters('reactivepro_get_review_grid_data', array(
            'allPosts'          => $results,
            'allAttachmentsUrl' => array()
        ));
    }
    ...
    public function get_bp_group_grid_data($wpml_lang)
    {
      ...
      WHERE ID <> %d", 0);
        ...
        return apply_filters('reactivepro_get_bp_group_grid_data', array(
            'allPosts'          => $results,
            'allAttachmentsUrl' => array()
        ));
    }
    ...
    public function get_bp_activity_grid_data($wpml_lang)
    {
      ...
      WHERE ID <> %d", 0);
        ...
        return apply_filters('reactivepro_get_bp_activity_grid_data', array(
            'allPosts'          => $results,
            'allAttachmentsUrl' => array()
        ));
    }
    ...
    GROUP BY meta_value, meta_key", $metakeys);
        ...
        return apply_filters('reactivepro_get_meta_to_user', $results);
    }
    ...
    GROUP BY meta_value, meta_key", $metakeys);
        ...
        return apply_filters('reactivepro_get_meta_to_review', $results);
    }
    ...
    GROUP BY meta_value, meta_key", $metakeys);
        ...
        return apply_filters('reactivepro_get_meta_to_bp_group', $results);
    }
    ...
    GROUP BY meta_value, meta_key", $metakeys);
        ...
        return apply_filters('reactivepro_get_meta_to_bp_activity', $results);
    }
}
