...
if (isset($builderpost->ID)) :
	...
	$wpml_lang = '';
	$wpml_default_lang = ''; // need this for turbowp
...
?>
		<div class="reactivePreviewBtns">
			<form method="post">
				<button type="submit" name="editMode" value="admin" class="reactiveAdminBtn <?php if ($editMode === 'admin') {
																																											echo 'active';
																																										} ?>">Admin View
				</button>
				<button type="submit" name="editMode" value="user" class="reactiveUserBtn <?php if ($editMode === 'user') {
																																										echo 'active';
																																									} ?>">User View
				</button>
			</form>
	...
	if (isset($builderdata->fetch_thumb_image)) {
		$fetch_thumb_image = $builderdata->fetch_thumb_image;
	}
	switch ($search_type) {
		case 'user':
		...
		default:
			...
			$all_terms          = class_exists('RedQ_Rental_And_Bookings') ? $graph->get_rnb_terms($taxonomies, $post_types, $wpml_lang, $search_restricted_terms) : $graph->get_all_terms($taxonomies, $post_types, $wpml_lang, $search_restricted_terms);
			...
			$meta_to_post       = class_exists('RedQ_Rental_And_Bookings') ? $graph->rnb_meta_to_post($metakeys, $wpml_lang, $post_types, $serialized_meta_keys) : $graph->get_meta_to_post($metakeys, $wpml_lang, $post_types, $serialized_meta_keys);
			$grid_all_data      = $graph->get_grid_all_data($post_types, $wpml_lang, $skip_post_content, $fetch_thumb_image);
			break;
	}
	...
	$alldata = array_merge($reactivedata ? json_decode(json_encode($reactivedata), true) : array(), array(
		'taxonomies'         => apply_filters('reactivepro_taxonomies', $taxonomies),
		'metakeys'           => apply_filters('reactivepro_metakeys', $metakeys),
		'termMetaKeys'       => apply_filters('reactivepro_term_meta_keys', $term_meta_keys),
		'sortingAttributes'  => apply_filters('reactivepro_sorting_attributes', $sorting_attributes),
		'allPostTypes'       => apply_filters('reactivepro_fromated_post_types', $fromated_post_types),
		// 'allPosts'       => $graph->allPosts($post_types, $wpml_lang),
		'allPosts'           => apply_filters('reactivepro_all_posts', $grid_all_data['allPosts']),
		'allterms'           => apply_filters('reactivepro_all_terms', $all_terms),
		'termToTerm'         => apply_filters('reactivepro_term_to_term', $term_to_term),
		// 'termToPost'     => $graph->get_term_to_post($taxonomies, $post_types, $wpml_lang),
		'postToTerms'        => apply_filters('reactivepro_post_to_terms', $post_to_terms),
		'metaToPost'         => apply_filters('reactivepro_meta_to_post', $meta_to_post),
		'taxonomyToTerm'     => apply_filters('reactivepro_taxonomy_to_terms', $taxonomy_to_terms),
		'allTermNames'       => apply_filters('reactivepro_all_term_names', $all_term_names),
		'termMetaToTerms'    => apply_filters('reactivepro_term_meta_to_terms', $term_meta_to_terms),
		'taxonomyNames'      => apply_filters('reactivepro_get_taxonomy_names', get_taxonomy_names($taxonomies)),
		'widgetsAreas'       => apply_filters('reactivepro_widget_areas', $widget_areas),
		'protocol'           => is_ssl() == true ? 'https' : 'http',
		'is_admin'           => $is_admin,
		'current_user'       => apply_filters('reactivepro_current_user', $current_user),
		'editMode'           => $editMode,
		'site_url'           => apply_filters('reactivepro_site_url', site_url()),
		'multi_lang'         => apply_filters('reactivepro_wpml_lang', $wpml_lang),
		'wpml_default_lang'	 => apply_filters('reactivepro_wpml_default_lang', $wpml_default_lang),
		'page_url'           => apply_filters('reactivepro_get_permalink', get_permalink()),
		'redirectPages'      => $redirectPages,
		// 'gridAllData'  => $graph->get_get_grid_all_data($post_types),
		'allAttachmentsUrl'  => apply_filters('reactivepro_all_attachments_url', $grid_all_data['allAttachmentsUrl']),
		'attachmentDir'      => $graph->get_attachment_dir_url(),
		'permalinkStructure' => $graph->get_permalink_structure($post_types),
		'allTemplates'       => apply_filters('reactivepro_all_templates', gridLeftSearchRight()),
		'selected_template'  => apply_filters('reactivepro_selected_template', $selected_template),
		'sortedData'         => apply_filters('reactivepro_sorted_data_array', $sorted_data_array),
		'allUsers'           => $fetch_user == 'true' ? $graph->get_all_users() : [],
		'categoryData'       => apply_filters('reactivepro_category_data', $category_data),
